...
def estimate_advanced_metrics(player_name, slg=0.0, hr_per_pa=0.0, iso=0.0):
    ...
    When Statcast data is unavailable, we derive values from the player's
    name so metrics remain unique across the dataset. This prevents everyone
    from sharing identical default numbers.
    """
    ...
    if iso > 0:
        ...
        # Fallback value when ISO is unavailable
        x_iso = 0.15 + (name_seed % 5) / 100.0
    # Add deterministic variation regardless of branch
